...
# 0.90은 너무 빡빡했다. 같은 액자라도 방 안 다른 위치에서 다시 보면 원근과
# 조명이 달라져 서명이 꽤 움직인다. 시드 1337에서 실측한 분포는 이렇다:
#   같은 사진 쌍(합쳐야 함):   n=7  최소 0.69  중앙값 0.79
#   다른 사진 쌍(합치면 안 됨): n=41 최대 0.65  중앙값 0.15
# 두 분포가 겹치지 않아 그 사이면 되지만, 여유가 0.04뿐이라 넉넉하지는
# 않다. 한 시드에서 잰 값이므로 중복이 다시 보이면 여기부터 의심할 것.
DEDUP_COS = 0.67
...
# eq=False: 필드에 numpy 배열이 있어서 dataclass가 만들어 주는 __eq__는
# 원소별 비교를 하고, `photo in room.photos` 같은 검사가 "truth value of an
# array is ambiguous"로 터진다. 사진은 동일성(identity)으로 비교하면 된다.
@dataclass(eq=False)
class Photo:
    """확정된 사진 한 장."""
    room_key: str
